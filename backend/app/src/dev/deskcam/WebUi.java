...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .viewport img { display:block; max-width:100%; max-height:100%; cursor:crosshair;
...
function frac(e) {
  ...
  return [clamp((e.clientX - b.left) / b.width, 0, 1),
          clamp((e.clientY - b.top) / b.height, 0, 1)];
}

/* The picture is letterboxed inside the viewport, so an overlay is placed against the
   image's own rectangle and not against the box around it. */
function imgBox() {
  const b = view.getBoundingClientRect();
  const v = view.parentNode.getBoundingClientRect();
  return {ox: b.left - v.left, oy: b.top - v.top, w: b.width, h: b.height};
}
...
function ringResult(fx, fy, j) {
  ...
  const label = locked && m.focus_diopters
    ? m.focus_metres_approx + ' m · ' + m.focus_diopters + ' d'
    : ((s === 'failed-locked' || s === 'unfocused') ? 'nothing to focus on here' : (s || 'done'));
  showRing(fx, fy, locked ? 'ok' : 'bad', label);
